...
exports.ContractGuardError = exports.REPORT_SCHEMA_VERSION = exports.MAX_REPORT_BYTES = exports.MAX_CANDIDATE_BYTES = void 0;
...
exports.shouldFailGate = shouldFailGate;
exports.createDefaultIdempotencyKey = createDefaultIdempotencyKey;
exports.readCandidate = readCandidate;
exports.validateReport = validateReport;
exports.runCheck = runCheck;
exports.canonicalReportUrl = canonicalReportUrl;
...
class ContractGuardError extends Error {
    status;
    code;
...
exports.ContractGuardError = ContractGuardError;
...
function shouldFailGate(gateResult, failOn) {
    if (failOn === 'never')
        return false;
    if (failOn === 'warnings')
...
function createDefaultIdempotencyKey(context) {
...
async function readCandidate(candidatePath, workspace) {
    const resolvedPath = node_path_1.default.resolve(candidatePath);
    const realPath = await node_fs_1.promises.realpath(resolvedPath).catch((error) => {
        throw new ContractGuardError(`candidate-path does not identify a readable file: ${resolvedPath}`, { cause: error });
    });
    if (workspace) {
        const realWorkspace = await node_fs_1.promises.realpath(node_path_1.default.resolve(workspace));
        const relative = node_path_1.default.relative(realWorkspace, realPath);
    ...
    const stats = await node_fs_1.promises.stat(realPath);
    if (!stats.isFile())
        throw new ContractGuardError('candidate-path must identify a regular file');
    if (stats.size < 1)
        throw new ContractGuardError('candidate contract must not be empty');
    if (stats.size > exports.MAX_CANDIDATE_BYTES) {
        throw new ContractGuardError(`candidate contract exceeds the ${exports.MAX_CANDIDATE_BYTES}-byte API limit`);
    }
    const extension = node_path_1.default.extname(realPath).toLowerCase();
    const contentTypes = {
    ...
    if (!contentType)
        throw new ContractGuardError('candidate-path must end in .json, .yaml, or .yml');
    const bytes = await node_fs_1.promises.readFile(realPath);
    return {
        bytes,
        filename: node_path_1.default.basename(realPath),
        contentType,
        sha256: (0, node_crypto_1.createHash)('sha256').update(bytes).digest('hex'),
        resolvedPath: realPath,
    };
}
function validateReport(value, expectedProjectId) {
    if (!value || typeof value !== 'object')
        throw new ContractGuardError('Contract Guard returned a non-object report');
    const report = value;
    if (report.schemaVersion !== exports.REPORT_SCHEMA_VERSION) {
    ...
    if (report.projectId !== expectedProjectId)
        throw new ContractGuardError('Contract Guard report projectId did not match the request');
    ...
    if (report.status !== 'completed')
        throw new ContractGuardError('Contract Guard returned a report that was not completed');
    ...
    if (!report.summary || typeof report.summary !== 'object')
        throw new ContractGuardError('Contract Guard report omitted its summary');
    const countKeys = [
        'breaking',
        'risky',
        'nonBreaking',
    ...
    if (typeof report.baselineContentHash !== 'string' ||
...
function endpointFor(apiUrl, projectId) {
...
async function safeApiError(response) {
    ...
    try {
        const body = (await readBoundedJson(response, 65_536));
        if (typeof body.error?.code === 'string')
            code = body.error.code.slice(0, 80);
...
async function readBoundedJson(response, maximumBytes) {
    ...
    if (!response.body)
        throw new ContractGuardError('Contract Guard returned an empty response', { status: response.status });
...
async function runCheck(request, dependencies = {
    ...
    for (let attempt = 1; attempt <= request.attempts; attempt += 1) {
        ...
        if (request.displayName)
            body.append('display_name', request.displayName);
        body.append('candidate', new Blob([new Uint8Array(request.candidate.bytes)], { type: request.candidate.contentType }), request.candidate.filename);
        ...
        try {
            ...
            if (response.status === 200) {
                let json;
                try {
                    json = await readBoundedJson(response, exports.MAX_REPORT_BYTES);
                }
                catch (error) {
...
function canonicalReportUrl(apiUrl, report) {
    ...
    if (reportUrl.origin !== base.origin)
        throw new ContractGuardError('Contract Guard returned a cross-origin reportUrl');
    return reportUrl.toString();
}
//# sourceMappingURL=contract-guard.js.map